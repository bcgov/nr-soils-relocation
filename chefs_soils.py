...
def map_source_site(_submission):
    ...
    if (helper.validate_lat_lon(_submission.get(helper.chefs_src_param('latitudeDegrees')), _submission.get(helper.chefs_src_param('latitudeMinutes')), _submission.get(helper.chefs_src_param('latitudeSeconds')), 
        ...
        _src_dic['contactPhoneNumber'] = \
          _submission.get(helper.chefs_src_param('contactPhoneNumber'))
        ...
        _src_dic['landOwnership'] = \
          helper.create_land_ownership(_submission, helper.chefs_src_param('landOwnership'))
        _src_dic['regionalDistrict'] = \
          helper.create_regional_district(_submission, helper.chefs_src_param('regionalDistrict'))
        _src_dic['legallyTitledSiteAddress'] = \
          _submission.get(helper.chefs_src_param('legallyTitledSiteAddress'))
        _src_dic['legallyTitledSiteCity'] = \
          _submission.get(helper.chefs_src_param('legallyTitledSiteCity'))
        _src_dic['legallyTitledSitePostalCode'] = \
          _submission.get(helper.chefs_src_param('legallyTitledSitePostalCode'))
        _src_dic['crownLandFileNumbers'] = \
          helper.create_land_file_numbers(
            _submission, 
            helper.chefs_src_param('crownLandFileNumbers'))

        _src_dic['PID'], _src_dic['legalLandDescription'] = \
          helper.create_pid_pin_and_desc(
            _submission,
            helper.chefs_src_param('pidDataGrid'),
            helper.chefs_src_param('pid'),
            helper.chefs_src_param('pidDesc'))  #PID
        if (_src_dic['PID'] is None or _src_dic['PID'].strip() == ''): #PIN
            _src_dic['PIN'], _src_dic['legalLandDescription'] = \
              helper.create_pid_pin_and_desc(
                _submission,
                helper.chefs_src_param('pinDataGrid'),
                helper.chefs_src_param('pin'),
                helper.chefs_src_param('pinDesc'))
        if ((_src_dic['PID'] is None or _src_dic['PID'].strip() == '')
            ...
            _src_dic['legalLandDescription'] = \
              helper.create_untitled_municipal_land_desc(
                _submission,
                helper.chefs_src_param('untitledMunicipalLand'),
                helper.chefs_src_param('untitledMunicipalLandDesc'))

        if _submission.get(helper.chefs_src_param('sourceSiteLandUse')) is not None \
          and len(_submission.get(helper.chefs_src_param('sourceSiteLandUse'))) > 0 :
            _source_site_land_uses = []
            for _ref_source_site in _submission.get(\
              helper.chefs_src_param('sourceSiteLandUse')):
                _source_site_land_uses.append(
                  helper.convert_source_site_use_to_name(_ref_source_site))
        ...
        _src_dic['soilRelocationPurpose'] = \
          _submission.get(helper.chefs_src_param('soilRelocationPurpose'))
        _src_dic['soilStorageType'] = _submission.get(helper.chefs_src_param('soilStorageType'))

        helper.create_soil_volumes(
          _submission,
          helper.chefs_src_param('soilVolumeDataGrid'),
          helper.chefs_src_param('soilVolume'),
          helper.chefs_src_param('soilClassificationSource'),
          _src_dic)

        _src_dic['soilCharacterMethod'] = \
          _submission.get(helper.chefs_src_param('soilCharacterMethod'))
        _src_dic['vapourExemption'] = \
          _submission.get(helper.chefs_src_param('vapourExemption'))
        _src_dic['vapourExemptionDesc'] = \
          _submission.get(helper.chefs_src_param('vapourExemptionDesc'))
        _src_dic['vapourCharacterMethodDesc'] = \
          _submission.get(helper.chefs_src_param('vapourCharacterMethodDesc'))
        _src_dic['soilRelocationStartDate'] = \
          helper.convert_simple_datetime_format_in_str(\
            _submission.get(helper.chefs_src_param('soilRelocationStartDate')))
        _src_dic['soilRelocationCompletionDate'] = \
          helper.convert_simple_datetime_format_in_str(\
            _submission.get(helper.chefs_src_param('soilRelocationCompletionDate')))
        _src_dic['relocationMethod'] = _submission.get(helper.chefs_src_param('relocationMethod'))
        _src_dic['qualifiedProfessionalFirstName'] = \
          _submission.get(helper.chefs_src_param('qualifiedProfessionalFirstName'))
        _src_dic['qualifiedProfessionalLastName'] = \
          _submission.get(helper.chefs_src_param('qualifiedProfessionalLastName'))
        _src_dic['qualifiedProfessionalType'] = \
          _submission.get(helper.chefs_src_param('qualifiedProfessionalType'))
        _src_dic['professionalLicenceRegistration'] = \
          _submission.get(helper.chefs_src_param('professionalLicenceRegistration'))
        _src_dic['qualifiedProfessionalOrganization'] = \
          _submission.get(helper.chefs_src_param('qualifiedProfessionalOrganization'))
        _src_dic['qualifiedProfessionalAddress'] = \
          _submission.get(helper.chefs_src_param('qualifiedProfessionalAddress'))
        _src_dic['qualifiedProfessionalCity'] = \
          _submission.get(helper.chefs_src_param('qualifiedProfessionalCity'))
        _src_dic['qualifiedProfessionalProvince'] = \
          _submission.get(helper.chefs_src_param('qualifiedProfessionalProvince'))
        _src_dic['qualifiedProfessionalCountry'] = \
          _submission.get(helper.chefs_src_param('qualifiedProfessionalCountry'))
        _src_dic['qualifiedProfessionalPostalCode'] = \
          _submission.get(helper.chefs_src_param('qualifiedProfessionalPostalCode'))
        _src_dic['qualifiedProfessionalPhoneNumber'] = \
          _submission.get(helper.chefs_src_param('qualifiedProfessionalPhoneNumber'))
        _src_dic['qualifiedProfessionalEmail'] = \
          _submission.get(helper.chefs_src_param('qualifiedProfessionalEmail'))
        _src_dic['signaturerFirstAndLastName'] = \
          _submission.get(helper.chefs_src_param('signaturerFirstAndLastName'))
        _src_dic['dateSigned'] = \
          helper.convert_simple_datetime_format_in_str(\
            _submission.get(helper.chefs_src_param('dateSigned')))
        _src_dic['createAt'] = helper.get_create_date(\
          _submission,
          helper.chefs_src_param('form'),
          helper.chefs_src_param('createdAt'))
...
def map_rcv_site(_submission, rcv_clz):
    ...
    if (helper.validate_additional_rcv_site(_submission, rcv_clz) and
        ...
        _rcv_dic['ownerFirstName'] = \
          _submission.get(helper.chefs_rcv_param('ownerFirstName', rcv_clz))
        _rcv_dic['ownerLastName'] = \
          _submission.get(helper.chefs_rcv_param('ownerLastName', rcv_clz))
        ...
        _rcv_dic['ownerProvince'] = \
          _submission.get(helper.chefs_rcv_param('ownerProvince', rcv_clz))
        _rcv_dic['ownerCountry'] = _submission.get(helper.chefs_rcv_param('ownerCountry', rcv_clz))
        _rcv_dic['ownerPostalCode'] = \
          _submission.get(helper.chefs_rcv_param('ownerPostalCode', rcv_clz))
        _rcv_dic['ownerPhoneNumber'] = \
          _submission.get(helper.chefs_rcv_param('ownerPhoneNumber', rcv_clz))
        _rcv_dic['ownerEmail'] = _submission.get(helper.chefs_rcv_param('ownerEmail', rcv_clz))
        _rcv_dic['owner2FirstName'] = \
          _submission.get(helper.chefs_rcv_param('owner2FirstName', rcv_clz))
        _rcv_dic['owner2LastName'] = \
          _submission.get(helper.chefs_rcv_param('owner2LastName', rcv_clz))
        _rcv_dic['owner2Company'] = \
          _submission.get(helper.chefs_rcv_param('owner2Company', rcv_clz))
        _rcv_dic['owner2Address'] = \
          _submission.get(helper.chefs_rcv_param('owner2Address', rcv_clz))
        _rcv_dic['owner2City'] = _submission.get(helper.chefs_rcv_param('owner2City', rcv_clz))
        _rcv_dic['owner2Province'] = \
          _submission.get(helper.chefs_rcv_param('owner2Province', rcv_clz))
        _rcv_dic['owner2Country'] = \
          _submission.get(helper.chefs_rcv_param('owner2Country', rcv_clz))
        _rcv_dic['owner2PostalCode'] = \
          _submission.get(helper.chefs_rcv_param('owner2PostalCode', rcv_clz))
        _rcv_dic['owner2PhoneNumber'] = \
          _submission.get(helper.chefs_rcv_param('owner2PhoneNumber', rcv_clz))
        _rcv_dic['owner2Email'] = _submission.get(helper.chefs_rcv_param('owner2Email', rcv_clz))
        _rcv_dic['additionalOwners'] = \
          _submission.get(helper.chefs_rcv_param('additionalOwners', rcv_clz))
        _rcv_dic['contactFirstName'] = \
          _submission.get(helper.chefs_rcv_param('contactFirstName', rcv_clz))
        _rcv_dic['contactLastName'] = \
          _submission.get(helper.chefs_rcv_param('contactLastName', rcv_clz))
        _rcv_dic['contactCompany'] = \
          _submission.get(helper.chefs_rcv_param('contactCompany', rcv_clz))
        _rcv_dic['contactAddress'] = \
          _submission.get(helper.chefs_rcv_param('contactAddress', rcv_clz))
        _rcv_dic['contactCity'] = _submission.get(helper.chefs_rcv_param('contactCity', rcv_clz))
        _rcv_dic['contactProvince'] = \
          _submission.get(helper.chefs_rcv_param('contactProvince', rcv_clz))
        _rcv_dic['contactCountry'] = \
          _submission.get(helper.chefs_rcv_param('contactCountry', rcv_clz))
        _rcv_dic['contactPostalCode'] = \
          _submission.get(helper.chefs_rcv_param('contactPostalCode', rcv_clz))
        _rcv_dic['contactPhoneNumber'] = \
          _submission.get(helper.chefs_rcv_param('contactPhoneNumber', rcv_clz))
        ...
        _rcv_dic['regionalDistrict'] = helper.create_regional_district(
          _submission,
          helper.chefs_rcv_param('regionalDistrict', rcv_clz))
        _rcv_dic['landOwnership'] = helper.create_land_ownership(
          _submission,
          helper.chefs_rcv_param('landOwnership', rcv_clz))

        _rcv_dic['legallyTitledSiteAddress'] = _submission.get(
          helper.chefs_rcv_param('legallyTitledSiteAddress', rcv_clz))
        _rcv_dic['legallyTitledSiteCity'] = _submission.get(
          helper.chefs_rcv_param('legallyTitledSiteCity', rcv_clz))
        _rcv_dic['legallyTitledSitePostalCode'] = _submission.get(
          helper.chefs_rcv_param('legallyTitledSitePostalCode', rcv_clz))
        ...
        if (_rcv_dic['PID'] is None or _rcv_dic['PID'].strip() == ''): #PIN
            _rcv_dic['PIN'], _rcv_dic['legalLandDescription'] = \
              helper.create_pid_pin_and_desc(
                _submission,
        ...
        if ((_rcv_dic['PID'] is None or _rcv_dic['PID'].strip() == '')
            and (_rcv_dic['PIN'] is None
                  or _rcv_dic['PIN'].strip() == '')): 
                  #Description when selecting 'Untitled Municipal Land'
            _rcv_dic['legalLandDescription'] = \
              helper.create_untitled_municipal_land_desc(
                  _submission,
                  helper.chefs_rcv_param('untitledMunicipalLand', rcv_clz),
                  helper.chefs_rcv_param('untitledMunicipalLandDesc', rcv_clz))

        _rcv_dic['crownLandFileNumbers'] = \
          helper.create_land_file_numbers(
            _submission,
            helper.chefs_rcv_param('crownLandFileNumbers', rcv_clz))
        _rcv_dic['receivingSiteLandUse'] = \
          helper.create_receiving_site_lan_uses(
            _submission,
            helper.chefs_rcv_param('receivingSiteLandUse', rcv_clz))

        _rcv_dic['CSRFactors'] = \
          _submission.get(helper.chefs_rcv_param('CSRFactors', rcv_clz))
        _rcv_dic['relocatedSoilUse'] = \
          _submission.get(helper.chefs_rcv_param('relocatedSoilUse', rcv_clz))
        _rcv_dic['highVolumeSite'] = \
          _submission.get(helper.chefs_rcv_param('highVolumeSite', rcv_clz))
        _rcv_dic['soilDepositIsALR'] = \
          _submission.get(helper.chefs_rcv_param('soilDepositIsALR', rcv_clz))
        _rcv_dic['soilDepositIsReserveLands'] = \
          _submission.get(helper.chefs_rcv_param('soilDepositIsReserveLands', rcv_clz))
        _rcv_dic['dateSigned'] = \
          helper.convert_simple_datetime_format_in_str(
            _submission.get(helper.chefs_rcv_param('dateSigned', rcv_clz)))
...
def map_hv_site(_hvs):
    ...
    if (helper.validate_lat_lon(
      ...
      'High Volume Receiving Site Form')
        ...
        _hv_dic['regionalDistrict'] = \
          helper.create_regional_district(_hvs, helper.chefs_hv_param('regionalDistrict'))
        _hv_dic['landOwnership'] = \
          helper.create_land_ownership(_hvs, helper.chefs_hv_param('landOwnership'))
        _hv_dic['legallyTitledSiteAddress'] = \
          _hvs.get(helper.chefs_hv_param('legallyTitledSiteAddress'))
        _hv_dic['legallyTitledSiteCity'] = \
          _hvs.get(helper.chefs_hv_param('legallyTitledSiteCity'))
        _hv_dic['legallyTitledSitePostalCode'] = \
          _hvs.get(helper.chefs_hv_param('legallyTitledSitePostalCode'))
        ...
        if (_hv_dic['PID'] is None or _hv_dic['PID'].strip() == ''): #PIN
            _hv_dic['PIN'], _hv_dic['legalLandDescription'] = \
              helper.create_pid_pin_and_desc(
                _hvs,
        ...
        if ((_hv_dic['PID'] is None or _hv_dic['PID'].strip() == '')
            ...
            #Description when selecting 'Untitled Municipal Land'
            _hv_dic['legalLandDescription'] = \
              helper.create_untitled_municipal_land_desc(
                _hvs,
                helper.chefs_hv_param('untitledMunicipalLand'),
                helper.chefs_hv_param('untitledMunicipalLandDesc'))

        _hv_dic['crownLandFileNumbers'] = \
          helper.create_land_file_numbers(_hvs, helper.chefs_hv_param('crownLandFileNumbers'))
        _hv_dic['receivingSiteLandUse'] = \
          helper.create_receiving_site_lan_uses(_hvs, helper.chefs_hv_param('receivingSiteLandUse'))
        _hv_dic['hvsConfirmation'] = _hvs.get(helper.chefs_hv_param('hvsConfirmation'))
        _hv_dic['dateSiteBecameHighVolume'] = \
          helper.convert_simple_datetime_format_in_str(
            _hvs.get(helper.chefs_hv_param('dateSiteBecameHighVolume')))
        _hv_dic['howRelocatedSoilWillBeUsed'] = \
          _hvs.get(helper.chefs_hv_param('howRelocatedSoilWillBeUsed'))
        _hv_dic['soilDepositIsALR'] = _hvs.get(helper.chefs_hv_param('soilDepositIsALR'))
        _hv_dic['soilDepositIsReserveLands'] = \
          _hvs.get(helper.chefs_hv_param('soilDepositIsReserveLands'))
        _hv_dic['qualifiedProfessionalFirstName'] = \
          _hvs.get(helper.chefs_hv_param('qualifiedProfessionalFirstName'))
        _hv_dic['qualifiedProfessionalLastName'] = \
          _hvs.get(helper.chefs_hv_param('qualifiedProfessionalLastName'))
        _hv_dic['qualifiedProfessionalType'] = \
          _hvs.get(helper.chefs_hv_param('qualifiedProfessionalType'))
        _hv_dic['qualifiedProfessionalOrganization'] = \
          _hvs.get(helper.chefs_hv_param('qualifiedProfessionalOrganization'))
        _hv_dic['professionalLicenceRegistration'] = \
          _hvs.get(helper.chefs_hv_param('professionalLicenceRegistration'))
        _hv_dic['qualifiedProfessionalAddress'] = \
          _hvs.get(helper.chefs_hv_param('qualifiedProfessionalAddress'))
        _hv_dic['qualifiedProfessionalCity'] = \
          _hvs.get(helper.chefs_hv_param('qualifiedProfessionalCity'))
        _hv_dic['qualifiedProfessionalProvince'] = \
          _hvs.get(helper.chefs_hv_param('qualifiedProfessionalProvince'))
        _hv_dic['qualifiedProfessionalCountry'] = \
          _hvs.get(helper.chefs_hv_param('qualifiedProfessionalCountry'))
        _hv_dic['qualifiedProfessionalPostalCode'] = \
          _hvs.get(helper.chefs_hv_param('qualifiedProfessionalPostalCode'))
        _hv_dic['qualifiedProfessionalPhoneNumber'] = \
          _hvs.get(helper.chefs_hv_param('qualifiedProfessionalPhoneNumber'))
        _hv_dic['qualifiedProfessionalEmail'] = \
          _hvs.get(helper.chefs_hv_param('qualifiedProfessionalEmail'))
        _hv_dic['signaturerFirstAndLastName'] = \
          _hvs.get(helper.chefs_hv_param('signaturerFirstAndLastName'))
        _hv_dic['dateSigned'] = helper.convert_simple_datetime_format_in_str(
          _hvs.get(helper.chefs_hv_param('dateSigned')))
        _hv_dic['createAt'] = helper.get_create_date(
          _hvs, helper.chefs_hv_param('form'), helper.chefs_hv_param('createdAt'))
...
def send_email_subscribers():
    ...
    for _subscriber in subscribersJson:
        ...
        if _subscriber.get("notificationSelection") is not None :
            ...
            if _notice_selection.get(\
              'notifyOnHighVolumeSiteRegistrationsInSelectedRegionalDistrict') is not None:
                if helper.is_boolean(\
                  _notice_selection['notifyOnHighVolumeSite\
                    RegistrationsInSelectedRegionalDistrict']):
                    _notify_hvs = _notice_selection['notifyOn\
                      HighVolumeSiteRegistrationsInSelectedRegionalDistrict']
            if _notice_selection.get('notifyOnSoilRelocations\
              InSelectedRegionalDistrict') is not None:
                if helper.is_boolean(\
                  _notice_selection['notifyOnSoilRelocationsInSelectedRegionalDistrict']):
                    _notify_soil_reloc = \
                      _notice_selection['notifyOnSoilRelocationsInSelectedRegionalDistrict']
        ...
        if (_subscriber_email is not None and _subscriber_email.strip() != '' and
                ...
                for _srd in _subscriber_regional_district:
                    ...
                    if _rcv_sites_in_rd is not None:
                        for _receiving_site_dic in _rcv_sites_in_rd:
                            ...
                            #script runtime.
                            _diff = helper.get_difference_datetimes_in_hour(\
                              _today, _receiving_site_dic['createAt'])
                            if (_diff is not None and _diff <= 24):  #within the last 24 hours.
                                ...
                                _email_msg = create_site_relocation_email_msg(
                                  _reg_dis_name, _rcv_popup_links)
                                ...
                                if (_subscriber_email,_srd) not in notify_soil_reloc_subscriber_dic:
                                    notify_soil_reloc_subscriber_dic[(_subscriber_email,_srd)] = \
                                      (_email_msg,
                                       _subscription_created_at,
                                       _subscription_confirm_id)
                                    ...
                                    _subscrb_created = \
                                      notify_soil_reloc_subscriber_dic\
                                        .get((_subscriber_email,_srd))[1]
                                    if (_subscription_created_at is not None \
                                      and _subscription_created_at > _subscrb_created):
                                        notify_soil_reloc_subscriber_dic\
                                          .update({(_subscriber_email,_srd):(
                                            _email_msg,
                                            _subscription_created_at,
                                            _subscription_confirm_id)})
            ...
            if _notify_hvs:
                        ...
                        for _hv_site_dic in _hv_sites_in_rd:
                            ...
                            if (_diff is not None and _diff <= 24):  #within the last 24 hours.
                                ...
                                if (_subscriber_email,_srd) not in notify_hvs_subscriber_dic:
                                    notify_hvs_subscriber_dic[(_subscriber_email,_srd)] = \
                                      (_hv_email_msg,
                                      _subscription_created_at,
                                      _subscription_confirm_id)
                                    ...
                                    _subscrb_created = \
                                      notify_hvs_subscriber_dic.get((_subscriber_email,_srd))[1]
                                    if (_subscription_created_at is not None \
                                      and _subscription_created_at > _subscrb_created):
                                        notify_hvs_subscriber_dic.update({(\
                                          _subscriber_email,_srd):(
                                          _hv_email_msg,
                                          _subscription_created_at,
                                          _subscription_confirm_id)})
              ...
              _subscriber_regional_district is not None and \
                len(_subscriber_regional_district) > 0 and
              _unsubscribe):
            ...
            for _srd in _subscriber_regional_district:
                if (_subscriber_email,_srd) not in unsubscribers_dic:
                    ...
                    if (_subscription_created_at is not None and\
                       _subscription_created_at > _unsubscrb_created):
                        unsubscribers_dic.update({(_subscriber_email,_srd):\
                          _subscription_created_at})
    ...
    for (_k1_subscriber_email,_k2_srd), _unsubscribe_create_at in unsubscribers_dic.items():
        if (_k1_subscriber_email,_k2_srd) in notify_soil_reloc_subscriber_dic:
            _subscribe_create_at = \
              notify_soil_reloc_subscriber_dic.get((_k1_subscriber_email,_k2_srd))[1]
            _subscribe_confirm_id = \
              notify_soil_reloc_subscriber_dic.get((_k1_subscriber_email,_k2_srd))[2]
            if (_unsubscribe_create_at is not None and \
              _subscribe_create_at is not None and \
                _unsubscribe_create_at > _subscribe_create_at):
    ...
    for _k, _v in notify_soil_reloc_subscriber_dic.items():
        ...
        if _ches_response is not None and _ches_response.status_code is not None:
            print("[INFO] CHEFS Email response: " + \
              str(_ches_response.status_code) + ", subscriber email: " + _k[0])

    print('Sending Notification of high volume site registration \
      in selected Regional District(s) ...')
    for _k, _v in notify_hvs_subscriber_dic.items():
        ...
        if _ches_response is not None and _ches_response.status_code is not None:
            print("[INFO] CHEFS Email response: " + str(_ches_response.status_code) + ", \
              subscriber email: " + _k[0])
...
if _hvCsvItem is None:
    ...
    if _hvLyrItem is None:
        ...
        print('Updated High Volume Receiving Site Feature Layer successfully: ' + \
          json.dumps(_hvLyrOverwriteResult))
