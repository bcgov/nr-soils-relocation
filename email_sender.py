...
NOTICE_STANDARD_TIME = os.getenv('NOTICE_STANDARD_TIME') # Notification processing interval time
                ...
                for _srd in _subscriber_regional_district:
                    ...
                    if _src_sites_in_rd is not None:
                        for _source_site_dic in _src_sites_in_rd:
                            logging.debug("today:%s, created at:%s confirm Id:%s", current_date, _source_site_dic['createAt'], _source_site_dic['confirmationId'])
                            #comparing the submission create date against the current script runtime.
                            _diff = helper.get_difference_datetimes_in_hour(current_date, _source_site_dic['createAt'])
                            if (_diff is not None and _diff <= int(NOTICE_STANDARD_TIME)):  #within the last this hours.
                    ...
                    if _rcv_sites_in_rd is not None:
                        for _receiving_site_dic in _rcv_sites_in_rd:
                            logging.debug("today:%s, created at:%s confirm Id:%s", current_date, _receiving_site_dic['createAt'], _receiving_site_dic['confirmationId'])
                            #comparing the submission create date against the current script runtime.
                            _diff = helper.get_difference_datetimes_in_hour(current_date, _receiving_site_dic['createAt'])
                            if (_diff is not None and _diff <= int(NOTICE_STANDARD_TIME)):  #within the last this hours.
                    ...
                    if _hv_sites_in_rd is not None:
                        for _hv_site_dic in _hv_sites_in_rd:
                            logging.debug("today:%s, created at:%s, confirm Id:%s",
                                current_date, _hv_site_dic['createAt'], _hv_site_dic['confirmationId'])
                            # comparing the submission create date against the current script runtime.
                            _diff = helper.get_difference_datetimes_in_hour(current_date, _hv_site_dic['createAt'])
                            if (_diff is not None and _diff <= int(NOTICE_STANDARD_TIME)):  #within the last this hours.
